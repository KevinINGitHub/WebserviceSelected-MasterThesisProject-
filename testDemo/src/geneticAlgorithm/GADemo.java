...
public class GADemo {
	...
	public static double sinFun(double x){
		double y=0;
		y=Math.sin(x);
		return y;
	}
	public static void geneticForSin(CodeParam cp){
		cp.setVarNum(1);
		double [][] ValRg={{0},{25},{20}};
		cp.setValRg(ValRg);
		cp.setChromLength(20);
		cp.setPopNum(20);
		cp.setGeneration(100);
		...
		for(int i=0;i<varVList.length;i++){
			functionVList[i]=GADemo.sinFun(varVList[i][0]);
			functionMap.put(mutChroms[i], functionVList[i]);
		}
        
		//Thought the genetic algorithm to obtain the maxV in the give interval
        //Ω´map.entrySet()◊™ªª≥…list  
        List<Map.Entry<int[], Double>> list = new ArrayList<Map.Entry<int[], Double>>(functionMap.entrySet());  
